...
export const getUserById = (req: Request, res: Response, next: NextFunction) => User.findById(req.params.userId)
  .then((user) => {
    if (!user) {
      throw new Error('Некорректный id или пользователь с данным id не найден');
    }
    res.send({ data: user });
  })
  .catch(next);
  ...
  return User.create({
    ...
    .catch((err) => {
      if (err.code === 409 || err.code === 11000) {
        return next(new Error('Попытка создания дублирующей записи!'));
      } return next(err);
    });
};
...
export const updateUser = (req: TEnhancedRequest, res: Response, next: NextFunction) => {
  ...
  User.findByIdAndUpdate(
    ...
    .catch(() => next(new Error('Некорректный id или пользователь с данным id не найден')));
};
...
export const updateAvatar = (req: TEnhancedRequest, res: Response, next: NextFunction) => {
  ...
  User.findByIdAndUpdate(
    ...
    .catch(() => next(new Error('Пользователь не найден!')));
};